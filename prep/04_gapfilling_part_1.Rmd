...
Here we read in the data saved in the previous script, match it to the ARTIS database by species and origin country (and sometimes also by exporter country), and then gapfill any remaining rows that do not have information on fishmeal type. 
...
test <- artis_trimmings_info_traded %>%
  filter(year == 2020, sciname == "sardinella") %>%
  mutate(trim_fm = product_weight_t*trim_prop_gf,
         whole_fm = product_weight_t*(1-trim_prop_gf))

sum(test$trim_fm) # 1708.212
sum(test$whole_fm) # 612.5822
612.5822/(612.5822+1708.212) # year, 26% from whole fish. It should probably be the other way around. 
...
test/6158133 # 0.1051275 got about 11% of fishmeal 2020 this way! That's pretty good!  
```
...
test <- artis_trimmings_info_sources %>% 
  filter(year == 2020, sciname == "sardinella") %>%
    mutate(trim_fm = product_weight_t*trim_prop_gf,
         whole_fm = product_weight_t*(1-trim_prop_gf))

sum(test$trim_fm) # 10167.98
sum(test$whole_fm) # 56710.42
56710.42/(56710.42+10167.98) # ok cool, 85% from whole fish for sardinella, this makes sense! 
...
tonnes_lit/fm_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # 0.3564168 ~ 36% matching. How does that compare to before? Before was 40%, because we used the average from both the literature and reports. Should we try that now for some gapfilling? 
...
tonnes_lit/fm_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # 0.2512893 ~ 25% matching.

qs::qsave(artis_trimmings_all_info, here("int/non_gapfilled_fishmeal_matched.qs")) # save this for SI
...
test <- artis_trimmings_all_info_1 %>% 
  filter(year == 2020, sciname == "sardinella") %>%
    mutate(trim_fm = product_weight_t*trim_prop_gf,
         whole_fm = product_weight_t*(1-trim_prop_gf))
...
artis_trimmings_all_info_1 %>% 
  filter(year == 2020) %>%
  pull(product_weight_t) %>%
  sum()/fm_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # 0.4012207 ~ 40% matching. Cool, the mean gapfilling fills ~4% for 2020
...
test_trim <- sum(test_all$trim_weight, na.rm = TRUE) # 731538.3
test_whole <- sum(test_all$whole_weight, na.rm = TRUE) # 1739232

test_whole/test_weight_2020 # 28% from whole fish 
test_trim/test_weight_2020 # 12% from trimmings
...
test <- artis_trimmings_all_info_3 %>% 
  filter(year == 2020, sciname == "sardinella") %>%
    mutate(trim_fm = product_weight_t*trim_prop_gf,
         whole_fm = product_weight_t*(1-trim_prop_gf))

sum(test$trim_fm) # 18818.44
sum(test$whole_fm) # 65044.68 # good! 
...
conversions <- fillet_artis %>% 
  group_by(year, source_country_iso3c, sciname, method, habitat) %>%
  summarise(fillet_tonnes_lw = sum(live_weight_t, na.rm = TRUE),
            fillet_tonnes_pw = sum(product_weight_t, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(filleted = "yes",
         conversion = fillet_tonnes_pw/fillet_tonnes_lw)
qs::qsave(conversions, here("int/fillet_conversions.qs"))

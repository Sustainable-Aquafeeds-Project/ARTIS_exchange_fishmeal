...
fillet_price_all_gf <- price_all_gf %>%
 ...
 mutate(trim_prop_gf_fin = case_when(
   filleted == "yes" & is.na(trim_prop_gf) ~ 1,
  ...
  dplyr::select(year, source_country_iso3c, exporter_iso3c, consumer_iso3c, dom_source, hs6, sciname, common_name, habitat, method, trim_prop_gf_fin, trim_liveweight, trim_product, wholefish_liveweight, wholefish_product, method_trim_gf) %>%
...
Read in forage fisheries data and join; Forage fish species list taken from Clawson et al. 2024 in press: Continued transitions from fish meal and oil in aquafeeds require close attention to habitat impact trade-offs
...
test <- fm_data %>%
  ...
  mutate(prop = tonnes/total) # cool. 11% of all fishmeal is trimmings derived from forage fish. Of the trimmings fish meal only, 30% is from forage fish. Probably from canning operations. 
...
fm_data_2020 <- fm_data %>%
  ...
  ungroup() %>%
  filter(year == 2020, forage_fish == 1, fishmeal_type == "trimmings", method != "aquaculture", product_weight_t > 0) # yep, the forage fish trimmings is mostly from anchovies and sardines. I wonder if this matches the market for canned fish? There are definitely trimmings from anchovy canning (heads, tails). From google it seems like 40-60% of a peruvian anchoveta are discarded when canning (if they aren't canned whole)... so it's not implausible to think that the trimmings from canning could be used for FM and this could be the result? 
# most whole fish production is peruvian anchoveta or blue whiting
# there has also been some major pushes for using anchovies for human consumption by the peruvian government. 


fm_data_2020 <- fm_data %>%
  group_by(year, source_country_iso3c, sciname, fishmeal_type, method, forage_fish) %>%
  summarise(product_weight_t = sum(product_weight_t, na.rm = TRUE),
            live_weight_t = sum(live_weight_t, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(year == 2020, forage_fish == 1, fishmeal_type == "whole fish", method != "aquaculture", product_weight_t > 0) %>%
...
fm_data_2020 <- fm_data %>%
  ...
  summarise(product_weight_t = sum(product_weight_t, na.rm = TRUE),
            live_weight_t = sum(live_weight_t, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(year == 2020,  method != "aquaculture", product_weight_t > 0, source_country_iso3c == "GMB") %>%
  left_join(countries, by = c("source_country_iso3c" = "iso3c")) 


test <- fm_data %>%
  filter(year == 2020, source_country_iso3c == "GMB")

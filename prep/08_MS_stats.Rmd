...
total_tonnes/fao_2020_w # 0.9601988; ~94% - pretty good! 
...
finfish_fm %>% filter(year == 1996, fishmeal_type == "wholefish") %>% pull(total) - finfish_fm %>% filter(year == 2020, fishmeal_type == "wholefish") %>% pull(total)

# 992521.1 - lost about 1 million tonnes of whole fish finfish fm from 1996 to 2020
...
### Total
(chn_cons <- trade_flows_consumption %>% filter(importer_iso3c == "CHN") %>% pull(product_weight_t) %>% sum()) # 2238024
chn_cons/total_tonnes # 0.439426

### Trimmings vs whole 
trade_flows_consumption %>% filter(importer_iso3c == "CHN") %>% group_by(fishmeal_type) %>% summarise(product = sum(product_weight_t)) %>% mutate(prop = product/2238024)

# A tibble: 2 × 3
#   fishmeal_type  product  prop
#   <chr>            <dbl> <dbl>
# 1 trimmings     1150572. 0.514
# 2 wholefish     1087452. 0.486
...
#   fishmeal_type product   prop
#   <chr>           <dbl>  <dbl>
# 1 trimmings     842480. 0.165 
# 2 wholefish     203741. 0.0399
...
#   fishmeal_type product   prop
#   <chr>           <dbl>  <dbl>
# 1 trimmings     312955. 0.0613
# 2 wholefish     897071. 0.176 
...
fm_data_2019 %>% filter(importer_iso3c == "CHN") %>%
  pull(live_weight_t) %>% sum() # 13852124

fm_data_2019 %>% filter(importer_iso3c == "CHN") %>%
  pull(product_weight_t) %>% sum() # 2238024

15845529/13852124 # 1.143906 pretty efficient... 

```

See how consumption and importing of fishmeal lines up with fed aquaculture production
...
import_trim_prop <- global_trim_imports/global_imports # 0.3721439 of imports are trimmings
...
```
 ...
 - look at EU as a whole for trimmings production and see if it imports more than the global share

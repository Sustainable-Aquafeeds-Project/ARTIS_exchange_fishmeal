...
# read in prepped ARTIS fishmeal data
top_species_by_country_2019 <- qs::qread(here("outputs/top_species_fm_2015_2019.qs"))

top_species_by_country_2019 %>% filter(fishmeal_type == "wholefish") %>% pull(product_weight_t) %>% sum() # 2259208
...
test <- top_species_by_country_2013 %>% group_by(source_country_iso3c) %>% summarise(n_distinct(sciname)) # seems to have worked
  ...
  sum(top_species_by_country_2013$product_weight_t) # 4983782; we get ~4.98 million tonnes. They say that the "official IFFO figure is 4.8 million tonnes. So we are within an acceptable range! What does FAO report for 2013? 4.95 million tonnes
  ...
  top_species_by_country_2013 %>% filter(fishmeal_type == "wholefish") %>% pull(product_weight_t) %>% sum() #  whole fish
  top_species_by_country_2013 %>% filter(fishmeal_type == "trimmings") %>% pull(product_weight_t) %>% sum() #  trimmings


  ## lets look at just the chordata and cepholopods 
top_species_by_country_2013 %>% filter(fishmeal_type == "trimmings", phylum == "chordata"|class == "cephalopoda") %>% pull(product_weight_t) %>% sum() #  from trimmings 

top_species_by_country_2013 %>% filter(fishmeal_type == "wholefish", phylum == "chordata"|phylum == "cephalopoda") %>% pull(product_weight_t) %>% sum() #  from whole fish. Ok this is closer to what IFFO reports. Just less fishmeal overall - which is fine. 

top_species_by_country_2013 %>% filter(phylum == "chordata"|class == "cephalopoda") %>% pull(product_weight_t) %>% sum() #  total products without shrimps/prawns
...
To figure out: 
 - How much trimmings fishmeal could be created if BAU? I.e., we keep whole fish production going, but just utilise fillet and shrimp trimmings to full extent. Could this extra trimmings FM make up for losses in whole fish reduction? 
...
fillet_trim_2013 <- extra_trimmings %>%
  ...
  ungroup() %>%
    mutate(trimmings_fm_t = processing_waste_t*0.2, # 0.2 taken from Sandstrom et al.  
         fishmeal_type = "trimmings")

sum(fillet_trim_2013$trimmings_fm_t) # 1473233 - 1.47 million tonnes; This is just additional fishmeal. Newton and Jackson say There is ~2.365 million unused by-product tonnes of fishmeal on average from 2009-2013. So we are underestimating. 
...
#   ungroup() %>%
#   mutate(unused_trimmings_t = unused_trimmings_t_lw*0.2)
# 
#   
# sum(extra_trimmings_2013$unused_trimmings_t, na.rm = TRUE) # 1214884; way lower; after accounting for what we already use. This is probably due to different allocation factors

```

## Current day (2015 - 2019) scenarios 
 - calcualte unused trimmings and potential fishmeal 
 - calculate fishmeal that could be created by redirecting whole fish to human consumption
  - calculate unused shrimp waste 

```{r}
############## Look at 2015-2019 period now

extra_trimmings_2019 <- extra_trimmings %>% 
  filter(year %in% c(2015:2019)) %>%
    group_by(year, source_country_iso3c, exporter_iso3c, sciname) %>%
  summarise(unused_trimmings_t_lw = sum(processing_waste_t, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(source_country_iso3c, exporter_iso3c, sciname) %>%
  summarise(unused_trimmings_t_lw = mean(unused_trimmings_t_lw, na.rm = TRUE)) %>%
  ungroup() %>%
...
unused_trimmings_total <- sum(extra_trimmings_2019$unused_trimmings_t, na.rm = TRUE) # 1272506

wholefish_already <- top_species_by_country_2019 %>% filter(fishmeal_type == "wholefish") %>% pull(product_weight_t) %>% sum() # 2254781
top_species_by_country_2019 %>% filter(fishmeal_type == "trimmings") %>% pull(product_weight_t) %>% sum() # 2854162

unused_trimmings_total/wholefish_already # 0.5643588 if we used fillet trimmings fully, that could cover ~56% of the demand

## ok what about unused shrimp byproducts?
...
extra_shrimp_trim_2019 <- all_shrimp_prod_fao %>%
  filter(year %in% c(2015:2019)) %>%
    group_by(year, source_country_iso3c, common_name) %>%
  summarise(unused_t_lw = sum(tonnes_lw, na.rm = TRUE)) %>%
  ungroup() %>%
  ...
  mutate(unused_trimmings_pw_t = unused_trimmings_t_lw*0.2) # conversion factor of 20% from Sandstrom et al

extra_shrimp_prod <- sum(extra_shrimp_trim_2019$unused_trimmings_pw_t) # 986263.8

extra_shrimp_prod + unused_trimmings_total # 2258769
...
extra_fillet_fishmeal <- top_species_by_country_2019 %>%
  ...
  dplyr::select(1:11, conversion_fillet_final) %>%
  ...
  mutate(extra_fm_weight = live_weight_for_extra_fm*0.2)

extra_wholefish_fm <- sum(extra_fillet_fishmeal$extra_fm_weight) # 839142.8 tonnes of fishmeal if we redirected all whole fish FM to human consumption

extra_wholefish_fm + sum(extra_trimmings_2019$unused_trimmings_t, na.rm = TRUE) # 2111648 total fishmeal from utilising fillets trimmings and redirecting forage fish to fillets and trimmings 

top_species_by_country_2019 %>% filter(fishmeal_type == "wholefish") %>% pull(product_weight_t) %>% sum() # 2254781 # whole fish production

2111648-2254781 # -143133 # produces 150k less tonnes of fishmeal than what we currently do for whole fish

sum(extra_fillet_fishmeal$fillet_weight) # 1988551 extra fillet tonnes
...
non_finfish_fm_total <- sum(non_finfish_fm$product_weight_t) # 117421.3

## fishmeal from trimmings already produced? 
trimmings_already <- top_species_by_country_2019 %>% filter(fishmeal_type == "trimmings") %>% pull(product_weight_t) %>% sum() # 2854162

extra_shrimp_prod + extra_wholefish_fm + unused_trimmings_total + trimmings_already # 5952074 NEW TOTAL


5952074 - top_species_by_country_2019 %>% pull(product_weight_t) %>% sum() # WOULD BE PRODUCING 843131.7 more tonnes than before

## so our potential production is WAY less than other estimates.

### There are much larger numbers in Newton and Jackson 2016 and Sandstrom 2022 for potential trimmings, why? 
...
finfish_capture <- capture_prod %>%
  ...
  mutate(fm_byproduct = tonnes_lw*(.415-0.04)*0.2) %>% # this is the conversion to FM used in sandstrom et al. 
...
sum(finfish_capture$fm_byproduct) + sum(finfish_aquaculture$fm_byproduct) # 10024276
extra_shrimp_prod # 986263.8
# ok if we subtract their estimate (1.9 million tonnes):
10024276 + extra_shrimp_prod - 1900000 # 9110540 # pretty close! And it does seem like at least sandstrom includes shrimp production.

# ok so the discrepancy is likely that they are using ALL capture and aquaculture production whereas we just look at what is plausible (i.e. unused TRADED fillets and shrimp waste)

# if we do it, then we get 
10024276 + extra_shrimp_prod - trimmings_already # 8156378 # subtract out our estimate of trimmings and we should get ~8.2 million EXTRA tonnes of fm
# so that means using the same methods we would get ~8.2 million tonnes from unused byproducts. This probably makes sense given that we have a higher estimate of trimmings derived fishmeal than previously reported.


extra_shrimp_prod + unused_trimmings_total + trimmings_already + wholefish_already # 7367712 NEW TOTAL

7367712-(trimmings_already + wholefish_already) # 2258770

```

Make a plot of the scenarios we just did above: 
...
wholefish_already + trimmings_already + unused_trimmings_total + extra_shrimp_prod # 7367712 ~ 7.36

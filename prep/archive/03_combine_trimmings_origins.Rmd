...
shea_int <- read.csv(here("int/material_origins_shea_2025.csv")) %>%
      mutate(trimmings_tonnes = NA, wholefish_tonnes = NA) %>%
...
shea_missing_comms <- shea_int %>%
  ...
  mutate(sciname = case_when(
   country == "Canada" & sciname == "auxis spp." ~ "auxis",
   country == "Canada" &   sciname == "oncorhynchus spp." ~ "oncorhynchus",
   country == "Canada" &   sciname == "opisthonema spp." ~ "opisthonema oglinum",
    country == "Ecuador" &  sciname == "auxis brachydorax" ~ "auxis thazard", # synonym
     country == "Ecuador" & sciname == "larimus spp." ~ "sciaenidae",
     country == "Ecuador" & sciname == "opisthonema spp." ~ "clupea pallasii", # FAO only reports pacific thread herring
   ...
   country == "Indonesia" &   sciname == "amblygaster spp." ~ "amblygaster sirm", # sardinella caught in Indonesia
    country == "Mauritania" & sciname == "sardinella spp." ~ "sardinella",
   country == "Mexico" & sciname == "etrumeus teres" ~ "etrumeus sadina", # synonym
   country == "Mexico" & sciname == "oligoplites spp." ~ "oligoplites refulgens", # reported in FAO catch
   country == "Mexico" & sciname == "opisthonema spp." ~ "opisthonema libertate", # FAO catch
   country == "Panama" & sciname == "opisthonema spp." ~ "opisthonema libertate", # FAO catch
   country == "United Kingdom" & sciname == "lepidorhombus spp." ~ "lepidorhombus",
   country == "United States of America" & sciname == "urophycis spp." ~ "urophycis tenuis", # FAO catch
   country == "Vietnam" & sciname == "pangasius hypophthalmas" ~ "pangasius",
      country == "Thailand" & sciname == "pangasius hypophthalmas" ~ "pangasius",
   TRUE ~ sciname
      )) %>%
  dplyr::select(-common_name) %>%
  left_join(., artis_comm) # a couple I couldn't fill.

shea_int_fin <- rbind(shea_int %>% filter(!is.na(common_name)), shea_missing_comms)


all_int_dfs <- rbind(biomar_int, cargill_int, skretting_int, cao_int, chiu_int, greenpeace_int, peron_int, edwards_int, havfisk_int, shea_int_fin) %>%
  mutate(sciname = tolower(sciname),
...
test_3 <- read.csv(here("outputs/all_fmfo_material_origins_reports.csv"))

...
write.csv(fm_origins_df_fin, here("int/fishmeal_origins_biomar_skretting_raw.csv"), row.names = FALSE)

## filter artis data for one word scinames

artis_family_names <- fm_artis %>% 
...
trim_family_names <- fm_origins_df_fin %>%
  ...
  mutate(sciname = ifelse(common_name %in% c("herrings, sardines nei"), "clupeidae,clupea,clupeiformes", sciname)) %>% # fix this one
      separate_rows(sciname, sep = ",")


test <- trim_family_names %>% 
  filter(is.na(sciname)) ## still missing one
...
trim_family_names_fin <- fm_origins_df_names %>%
  rbind(., trim_family_names) %>%
  dplyr::relocate(sciname)


write.csv(trim_family_names_fin, here("int/fishmeal_origins_biomar_skretting.csv"), row.names = FALSE)

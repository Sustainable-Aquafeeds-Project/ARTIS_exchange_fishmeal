...
```{r}
full_artis_new <- read.csv(file.path(artis_dir, "2024_09_19_consumption_midpoint_all_hs_all_years.csv")) # really big file, nearly 10 gb, takes awhile to load. We'll load it in full and filter for what we need below (fish meal and fillet hs codes)

## save as a qs for easier loading and data space

qs::qsave(full_artis_new, file.path(artis_dir, "2024_09_19_consumption_midpoint_all_hs_all_years.qs")) # cuts the file size by 9x


# lets explore the new data a bit
test <- full_artis_new %>% filter(year == 2020, end_use == "fishmeal", source_country_iso3c != consumer_iso3c, hs_version == 17) %>%
  mutate(product_weight = consumption_t_capped/2.975207)
sum(test$consumption_t_capped) # 9162738 - is this 9 million tonnes of fish meal or live weight? Let's check against what we pulled before
sum(test$product_weight) # 3079698 - ok this seems pretty close to what we had before.. off by ~500k tonnes 

old_fm <- qs::qread(file.path(artis_dir, "full_artis_fm_fillet.qs")) %>%
  filter(year == 2020, hs6 == 230120)

---
title: "Pull ARTIS data"
output: html_document
date: "2024-06-12"
---

## Summary

Here we pull fishmeal and fillet information out of the main, large ARTIS database and save to the repository. 

We will use the example script that Jessica has provided to pull trade information for fish meal and fish fillets 
 - https://github.com/Seafood-Globalization-Lab/exploreARTIS/blob/main/scripts/filter_bulk_artis.R

## Setup

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

library(here)
library(tidyverse)
library(qs)
library(arrow)

rdsi_dir <- file.path("/mnt/rdsi/raw_data")

artis_dir <- file.path(rdsi_dir, "ARTIS")

codes <- read.csv(file.path(artis_dir, "artis_full_data/data/attribute tables/products.csv")) # this is taken from ARTIS KNB
```

I've received an updated version of the database directly from Jessica Gephart (`gephart@uw.edu`), which contains the domestic production and consumption of fish meal. We will use this instead of the provided on KNB (see below) so that we can incorporate domestic production and consumption fish meal (e.g., how much fish meal does China produce DOMESTICALLY and consume, or not trade).

```{r}
full_artis_new <- qs::qread(file.path(artis_dir, "2024_09_19_consumption_midpoint_all_hs_all_years.qs")) # really big file, nearly 10 gb, takes awhile to load. We'll load it in full and filter for what we need below (fish meal and fillet hs codes)


# lets explore the new data a bit
test <- full_artis_new %>% filter(year == 2020, end_use %in% c("fishmeal", "other"), source_country_iso3c != consumer_iso3c, hs_version == 12) %>%
  mutate(product_weight = consumption_t_capped/2.975207)
sum(test$consumption_t_capped) # 9303891 -  9 million tonnes of live weight? Let's check against what we pulled before
sum(test$consumption_t) # 9425558
sum(test$product_weight) # 3127141 - ok this seems pretty close to what we had before with the traded only data.


test2 <- full_artis_new %>% filter(end_use == "other", year == 2020, hs_version == 17)
test3 <- full_artis_new %>% filter(end_use == "direct human consumption", year == 2020, hs_version == 17)
sum(test3$consumption_t_capped) # 165401647

9162738/165401647 # 0.0553969 ~ 5.5% of tonnage is fish meal

## ok this makes me think the new data is tonnes liveweight. Which is fine. ARTIS just applies a basic 2.975207x multiplier for the conversion from live weight to product weight

# Select set of years and hs versions of interest
hs_years <- data.frame(
  hs_version_new = c(
    # Length in each should match the set of selected years
    rep(96, length(1996:2003)),
    rep(2, length(2004:2009)),
    rep(7, length(2010:2012)),
    rep(12, length(2013:2016)),
    rep(17, length(2017:2020))
  ), 
  year = c(
    # HS96 years
    1996:2003,
    # HS02 years
    2004:2009,
    # HS07 years
    2010:2012,
    # HS12 years
    2013:2016,
    # HS17 years
    2017:2020
  )) 

# Subset file_list to the set of HS versions and years of interest
full_data_new <- full_artis_new %>%
  left_join(hs_years,
            by = c("year")) %>% 
  filter(hs_version == hs_version_new)

## lets just save the fishmeal part of this 

full_data_fm <- full_data_new %>%
  filter(end_use == "fishmeal") %>% 
  mutate(product_weight_t = consumption_t_capped/2.975207)

qs::qsave(full_data_fm, file.path(artis_dir, "full_artis_fm_2024_09_19.qs"))

full_data_human <- full_data_new %>%
  filter(end_use == "direct human consumption")

test <- full_data_human %>%
  filter(year == 2020)

qs::qsave(full_data_human, file.path(artis_dir, "full_artis_human_fm_2024_09_19.qs")) # save this incase we need the human consumption part

```


Save ARTIS data from KNB

Pulling and saving full ARTIS database provided on KNB: 
 - https://knb.ecoinformatics.org/view/doi:10.5063/F1CZ35N7 

```{r}
# Example script for loading and saving full ARTIS data from KNB

fillet_codes <- codes %>% 
  filter(presentation == "fillet") %>%
      mutate(hs6 = as.character(hs6)) %>%
    mutate(hs6 = case_when(
      str_length(hs6) == 5 ~ paste("0", hs6, sep = ""),
      TRUE ~ hs6
    )) %>%
  pull(hs6) %>%
  unique()


# Set data directory where artis data is loaded
datadir <- file.path(artis_dir, "artis_full_data/data/trade") 

intdir <- "int"

# List files in folder
file_list <- list.files(datadir)

# Select set of years and hs versions of interest
hs_years <- data.frame(
  hs_version = c(
    # Length in each should match the set of selected years
    rep("HS96", length(1996:2003)),
    rep("HS02", length(2004:2009)),
    rep("HS07", length(2010:2012)),
    rep("HS12", length(2013:2020))
    # Note: HS17 is not in the current custom ARTIS, but this would be included
    # if an analysis does include HS17 
    #rep("HS17", length(1996:2003))
  ), 
  year = c(
    # HS96 years
    1996:2003,
    # HS02 years
    2004:2009,
    # HS07 years
    2010:2012,
    # HS12 years
    2013:2020
    # HS17 years
  )) 

# Subset file_list to the set of HS versions and years of interest
file_list <- hs_years %>%
  left_join(as.data.frame(file_list) %>%
              separate(file_list, 
                       into = c("data", "est", "hs_version", "year", "csv")) %>%
              mutate(year = as.integer(year)),
            by = c("hs_version", "year")) %>%
  mutate(file_names = paste(data, est, hs_version, year, sep = "_")) %>%
  mutate(file_names = paste0(file_names, ".csv")) %>%
  pull(file_names)

# Loop through selected files and filter each column as desired
# before building back up the file for analysis

# Create empty data frame to build up
df <- data.frame(hs_version = c(), year = c(), 
                 source_country_iso3c = c(), exporter_iso3c = c(), 
                 importer_iso3c = c(), dom_source = c(), 
                 hs6 = c(), sciname = c(), habitat = c(), method = c(), 
                 product_weight_t = c(), live_weight_t = c())

for(i in 1:25){
  # i = 1
  
  print(paste(i, "of", length(file_list), file_list[i], sep = " "))
  
  df_i <- read.csv(file.path(datadir, file_list[i])) %>%
    # Source country
    # Uncomment and add any iso3c codes to filter by
    #filter(source_country_iso3c %in% c()) %>%
    
    # Exporter
    # Uncomment and add any iso3c codes to filter by
    #filter(exporter_iso3c %in% c()) %>%
    
    # Importer
    # Uncomment and add any iso3c codes to filter by
    #filter(importer_iso3c %in% c()) %>%
    
    # HS6 code
    # Add lead zeros to filter hs6 codes 
    # (do this regardless of whether you filter by hs6 codes)
    mutate(hs6 = as.character(hs6)) %>%
    mutate(hs6 = case_when(
      str_length(hs6) == 5 ~ paste("0", hs6, sep = ""),
      TRUE ~ hs6
    )) %>%
    # Uncomment and add any hs6 codes to filter by
    filter(hs6 %in% c("230120", fillet_codes)) %>%
    
    # Export source (domestic vs foreign vs unknown)
    # Uncomment and add any export sources to filter by
    #filter(dom_source %in% c()) %>%
    
    # Scientific names
    # Uncomment and add any scientific names to filter by
    #filter(sciname %in% c()) %>%
    
    # Habitat
    # Uncomment and add any habitats to filter by
    #filter(habitat %in% c()) %>%
    
    # Method
    # Uncomment and add any methods to filter by
    #filter(method %in% c()) %>%
    select(hs_version, year, 
           source_country_iso3c, exporter_iso3c, importer_iso3c, dom_source, 
           hs6, sciname, habitat, method, 
           product_weight_t, live_weight_t)
  
  df <- df %>%
    bind_rows(df_i)
}

## save to rdsi server 
qs::qsave(df, file.path(artis_dir, "full_artis_fm_fillet.qs"))

```



NOTE: Below this, we do not actually use any of this data. 
I've received another updated version of the database, and we will explore using this 

```{r}
# artis_ds <- open_dataset(file.path(artis_dir, "ARTIS_1.1.0_FAO_2025_08_02/datasets/ARTIS_consumption_v1.1.0_FAO_mid_2025-08-02.parquet"))
# 
# glimpse(artis_ds)
# names(artis_ds)
# 
# artis_filtered <- artis_ds %>% 
#     filter(
#     (hs_version == "HS96" & year <= 2003) |
#     (hs_version == "HS02" & year >= 2004 & year <= 2009) |
#     (hs_version == "HS07" & year >= 2010 & year <= 2012) |
#     (hs_version == "HS12" & year >= 2013 & year <= 2020)
#   )
# 
# full_artis_new <- artis_filtered %>%
#   collect()
# 
# # lets explore the new data a bit
# test <- artis_filtered %>% 
#   filter(year == 2020, 
#          end_use %in% c("fishmeal", "other"),
#          source_country_iso3c != consumer_iso3c) %>%
#   mutate(product_weight = consumption_live_t/2.975207) %>%
#   collect()
# 
# sum(test$consumption_live_t) # 4957410
# sum(test$product_weight) # 1666240 - really low
# 
# test <- artis_filtered %>% 
#   filter(year == 2020, 
#          end_use %in% c("fishmeal", "other")) %>%
#   mutate(product_weight = consumption_live_t/2.975207,
#          live_weight = consumption_live_t*2.975207) %>%
#   collect()
# 
# sum(test$consumption_live_t) # 5069555
# sum(test$product_weight) # 1703934 - really low - what is happening? Should be ~5 million tonnes product weight for 2020 
# sum(test$live_weight)
# 
# test <- artis_filtered %>% 
#   filter(year == 2020, 
#          end_use %in% c("fishmeal", "other"), 
#                   source_country_iso3c == consumer_iso3c) %>%
#   mutate(product_weight = consumption_live_t/2.975207,
#          live_weight = consumption_live_t*2.975207) %>%
#   collect()
# sum(test$consumption_live_t) # 112145 - hmmmm 
# 
# 
# test <- full_artis_new %>%
#   filter(end_use == "fishmeal")
# 
# old_fm <- qs::qread(file.path(artis_dir, "full_artis_fm_fillet.qs")) %>%
#   filter(year == 2020, hs6 == 230120, hs_version == "HS12")
# 
# test2 <- old_fm %>% filter(source_country_iso3c == importer_iso3c)
# sum(test2$live_weight_t) # 155710.8
# 
# sum(old_fm$live_weight_t) # 10397265
# sum(old_fm$product_weight_t) # 3494636
# 
# ## ok this makes me think the new data is tonnes product weight andn ot liveweight? Which is fine. I think Jessica just applies a basic 2.975207x multiplier for the conversion. 
# 
# ## lets just save the fishmeal part of this 
# 
# full_data_fm <- full_artis_new %>%
#   filter(end_use %in% c("fishmeal", "other")) %>% 
#   # mutate(live_weight_t = consumption_live_t*2.975207) %>%
#   # rename(product_weight_t = consumption_live_t) %>%
#   rename(live_weight_t = consumption_live_t) %>% 
#   mutate(product_weight_t = live_weight_t/2.975207) %>%
#   dplyr::select(-consumption_source, -consumption_live_t_capped, -consumption_percap_live_kg, -consumption_percap_live_kg_capped)
# 
# # old_fm <- qs::qread(file.path(artis_dir, "full_artis_fm_fillet.qs")) %>%
# #   filter(hs6 == 230120) # compare; new data has ~4.1 million rows, old data has 3.8 million rows.. makes sense there would be more rows in the new data, given there is domestic production and consumption included here 
# 
# test <- full_data_fm %>% filter(sciname == "salmo salar")
# 
# qs::qsave(full_data_fm, file.path(artis_dir, "full_artis_fm_2025_08_12.qs"))
# 
# full_data_fm_old  <- qs::qread(file.path(artis_dir, "full_artis_fm_2024_09_19.qs")) # the old data had a lot more rows, meaning probably had a bigger diversity of species.... our results will probably change quite a bit. 
# 
# full_data_fm <- qs::qread(file.path(artis_dir, "full_artis_fm_2025_08_12.qs")) %>%
#   filter(year == 2019) %>%
#   group_by(source_country_iso3c, sciname, method) %>%
#   summarise(total = sum(product_weight_t, na.rm = TRUE)) %>%
#   ungroup()
# # no whiteleg shrimp, no salmon, no tuna. 
# 
# test <- full_data_fm %>%
#   filter(sciname %in% c("katsuwonus pelamis", "salmo salar", "litopenaeus vannamei", "thunnus albacares", "clupea harengus", "cyprinus carpio", "penaeus monodon", "acetes japonicus", "dosidicus gigas", "illex argentinus", "oreochromis niloticus"))
# 
# sum(full_data_fm$total) # 1737653, way low
# 
# ## IFFO suggests that salmon byproducts are ~1.5% of fishmeal, tuna byproducts ~6.24% of fishmeal, tilapia is ~2%, and these are all missing from the new data. Seems like the byproducts are not in the new data, but were in the old? 

```

Now we need to prep new fillet data. We will use the trade data for this, instead of the consumption, as it should contain actual hs codes. 

```{r}
# artis_ds <- open_dataset(file.path(artis_dir, "ARTIS_1.1.0_FAO_2025_08_02/datasets/ARTIS_trade_v1.1.0_FAO_mid_2025-08-02.parquet"))
# 
# glimpse(artis_ds)
# names(artis_ds)
# 
# artis_filtered <- artis_ds %>% 
#     filter(
#     (hs_version == "HS96" & year <= 2003) |
#     (hs_version == "HS02" & year >= 2004 & year <= 2009) |
#     (hs_version == "HS07" & year >= 2010 & year <= 2012) |
#     (hs_version == "HS12" & year >= 2013 & year <= 2020)
#   )
# 
# full_artis_new <- artis_filtered %>%
#   collect()
# 
# 
# fillet_codes <- codes %>% 
#   filter(presentation == "fillet") %>%
#       mutate(hs6 = as.character(hs6)) %>%
#     mutate(hs6 = case_when(
#       str_length(hs6) == 5 ~ paste("0", hs6, sep = ""),
#       TRUE ~ hs6
#     )) %>%
#   pull(hs6) %>%
#   unique()
# 
# 
# full_artis_fillet <- full_artis_new %>% 
#   filter(hs6 %in% c(fillet_codes))
# 
# old_fillet_data <- qs::qread(file.path(artis_dir, "full_artis_fm_fillet.qs")) %>%
#   filter(hs6 %in% c(fillet_codes))
# test <- old_fillet_data %>%
#   filter(source_country_iso3c == importer_iso3c)
# 
# qs::qsave(full_artis_fillet, file.path(artis_dir, "full_artis_fillet_08_12_2025.qs"))

```



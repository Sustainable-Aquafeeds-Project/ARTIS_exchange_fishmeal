...
Based on this, we will assume that for finfish and non-bilvalve molluscs (i.e. squids and octopus), a 2% blood loss and 2% processing loss, everything else (namely shrimp/prawns) just a 2% processing loss (since they don't have blood)
...
qs::qsave(fm_data_adjusted, file.path(artis_dir, "artis_exchange/artis_fishmeal_trimmings_traded_consumption_adjusted_final_19-06-2025.qs"))
...
```{r}
  ...
  sum(trade_flows_average$product_weight_t) # 5513640 makes sense
  ...
  sum(top_species_by_country$product_weight_t) # 5018148 ; should be less and it is
  ...
  sum(trade_flows_average$product_weight_t) # 5530582 makes sense
  ...
  sum(top_species_by_country$product_weight_t) # 5071250 ; should be less and it is
...
fm_data_top_spp %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # 5159890
fm_data %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum()
5159890/6233093 # 0.8278218; we retain ~83% of fishmeal reported in ARTIS by only keeping top 20 species.
...
ggplot(overtime_all, aes(x = year, y = product_t, color = fishmeal_type)) +
  ...
  theme(legend.position = "bottom") +
    geom_text(data = filter(overtime_all, year == 2020, fishmeal_type == "trimmings", data_type != "Cephalopoda"),
      ...
      geom_text(data = filter(overtime_all, year == 2020, fishmeal_type == "whole fish", data_type != "Cephalopoda"),
            aes(x = year, y = product_t, label = perc),
            vjust = 0, hjust = -0.1, show.legend = FALSE, size = 3) +
      geom_text(data = filter(overtime_all, year %in% c(2020), fishmeal_type == "trimmings", data_type == "Cephalopoda"),
            aes(x = year, y = product_t, label = perc),
      ...
      geom_text(data = filter(overtime_all, year == 1996, fishmeal_type == "trimmings", data_type != "Crustaceans"),
            aes(x = year, y = product_t, label = perc),
            vjust = -0.9, hjust = 0.7, show.legend = FALSE, size = 3) +
...
5159890/5320713 # 0.9697742; 97% - pretty good! 
  ...
  for(i in 1:length(cols)){
    ...
    node_i <- links %>%
      ...
      mutate(name_new = case_when(
        prop <= 0.03203364617867 ~ "Other", # this is the cutoff for including Chile on the left hand side
        TRUE ~ col_i
      ))
...
test %>% filter(importer_iso3c == "CHN") %>% pull(tonnes) %>% sum()/sum(test$tonnes) # 0.4949704; ~49% of imports go to China. The scale on the plot makes it seem more like 80%? It's because the scale is out of 1 million. So that is millions of tonnes, not a proportion.
...
### Total
trade_flows_consumption %>% filter(importer_iso3c == "CHN") %>% pull(product_weight_t) %>% sum() # 2265728
2265728/total_tonnes # 0.446779

### Trimmings vs whole 
trade_flows_consumption %>% filter(importer_iso3c == "CHN") %>% group_by(fishmeal_type) %>% summarise(product = sum(product_weight_t)) %>% mutate(prop = product/2265728)

# A tibble: 2 × 3
#   fishmeal_type  product  prop
#   <chr>            <dbl> <dbl>
# 1 trimmings     1250161. 0.552
# 2 whole fish    1015566. 0.448
...
#   fishmeal_type product   prop
#   <chr>           <dbl>  <dbl>
# 1 trimmings     887701. 0.175 
# 2 whole fish    209718. 0.0414
...
sum(extra_trimmings_2013$unused_trimmings_t, na.rm = TRUE) # 2175761; pretty close; after accounting for what we already use
...
sum(extra_trimmings_2019$unused_trimmings_t, na.rm = TRUE) # 1863203

top_species_by_country_2019 %>% filter(fishmeal_type == "whole fish") %>% pull(product_weight_t) %>% sum() # 2114238

sum(extra_trimmings_2019$unused_trimmings_t, na.rm = TRUE)/2114238 # if we used fillet trimmings fully, that could cover ~88% of the demand
...
sum(extra_fillet_fishmeal$extra_fm_weight) # 1387774 tonnes of fishmeal if we redirected all whole fish FM to human consumption

1387774 + 1863203 # 3250977 total fishmeal from utilising fillets trimmings and redirecting forage fish
top_species_by_country_2019 %>% filter(fishmeal_type == "whole fish") %>% pull(product_weight_t) %>% sum() # 2114238

3250977-2114238 # 1136739

sum(extra_fillet_fishmeal$fillet_weight) # 1960846 extra fillet tonnes

fillet_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum()

1960846/3565793 # 0.5499046
...
top_species_by_country_2019 %>% filter(fishmeal_type == "trimmings") %>% pull(product_weight_t) %>% sum() # 2957012

# unused trimmings = 1863203
# redirected whole fish fillet trimmings = 1387774

1387774 + 1857602 + 2957012 + 9577.311 # 6211965 NEW TOTAL

sum(top_species_by_country_2019$product_weight_t)
# OLD TOTAL = 5071250

6211965 - 5071250 # WOULD BE PRODUCING 1140715 more tonnes than before
...
shrimp_prod <- aqua_prod %>%
  ...
  filter(year %in% c(2015:2019),
         value > 0) %>%
  mutate(trim_lw = value*0.55*0.98) %>% # 55% yield, 2% loss from processing
...
sum(extra_shrimp_trim_2019$unused_trimmings_t) # 392702.1



1387774 + 1857602 + 2957012 + 9577.311 + 392702.1 # 6604667 NEW TOTAL

# OLD TOTAL = 5071250

6604667 - 5071250 # WOULD BE PRODUCING 1533417 more tonnes than before

## so our potential production is ~3 million tonnes less than other estimates. I wonder if we shouldn't account for trimmings already included? 
...
unique(chn_aquaculture_finfish_shrimp$isscaap_group_en)
sum(chn_aquaculture_finfish_shrimp$live_weight_t) # 65513435

top_species_by_country_2019 <- qs::qread(here("outputs/top_species_fm_2015_2019.qs")) 

top_species_by_country_2019 %>% filter(importer_iso3c == "CHN") %>%
  pull(live_weight_t) %>% sum() # 6846208

top_species_by_country_2019 %>% filter(importer_iso3c == "CHN") %>%
  pull(product_weight_t) %>% sum() # 2301086

65513435/6846208 # 9.569302 pretty efficient... 

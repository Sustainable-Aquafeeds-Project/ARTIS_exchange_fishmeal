...
artis_trimmings_all <- qs::qread(file.path(artis_dir, "artis_exchange/artis_fishmeal_trimmings_simple_gf_traded_consumption.qs"))
...
## wow a pretty significant portion (~60%) of whole fish rendered fish meal is from species used for fillets
# ~78% of trimmings fish meal are also used as fillets, so that makes sense
...
There are still 648 species in the ARTIS data that don't have prices. We will gapfill the one word species that don't match (i.e., their genus) to the average of all species in that genus that are in the price data.
...
missing_taxa <- data.frame(sciname= setdiff(unique(ex_vessel_taxa$sciname), unique(artis_trimmings_fillet$sciname))) # 382 spp in ex vessel dasta and not in ARTIS
missing_taxa_artis <- data.frame(sciname=setdiff(unique(artis_trimmings_fillet$sciname), unique(ex_vessel_taxa$sciname))) # 648 spp in artis not in price
...
missing_taxa <- data.frame(sciname= setdiff(unique(ex_vessel_taxa$sciname), unique(artis_trimmings_fillet$sciname))) # 382 spp
missing_taxa_artis <- data.frame(sciname=setdiff(unique(artis_trimmings_fillet$sciname), unique(ex_vessel_taxa$sciname))) # 601 spp
...
qs::qsave(artis_prices_gf, here("int/artis_trimmings_fillet_prices_consumption.qs")) 
```
...
trim_mean_price <- artis_prices_gf %>%
  filter(trim_prop_gf > 0.5) %>%
  pull(price_gf) %>%
  mean(., na.r = TRUE)

artis_finfish <- artis_prices_gf %>% 
  mutate(trim_prop_gf = ifelse(price_gf <= trim_mean_price & is.na(trim_prop_gf), 0, trim_prop_gf)) %>%
   mutate(trim_prop_gf = ifelse(price_gf > trim_mean_price & is.na(trim_prop_gf), 1, trim_prop_gf)) %>%

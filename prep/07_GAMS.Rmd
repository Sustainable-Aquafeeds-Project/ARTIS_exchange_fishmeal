...
```{r}
ctrl <- list(nthreads=20) # 20 cores?
...
for (i in 1:k) {
  ...
  # Align levels of source_country_iso3c
  if ("source_country_iso3c" %in% names(train)) {
    test <- test %>% 
      filter(source_country_iso3c %in% unique(train$source_country_iso3c))
  }
  
  for (formula in choose_models) {
    # formula = choose_models[5]
    
    # Skip if result for this formula and fold already exists
    output_path <- here(glue("int/models/diagnostics/cv_results/{formula}_fold_{k}_{i}_results.qs"))
    if (file.exists(output_path)) {
      cat(sprintf("Skipping formula '%s' for fold %d as results already exist.\n", formula, i))
      next()
    }
    ...
    model <- tryCatch(
      gam(as.formula(formula), data = train, family = betar(), control = ctrl),
      error = function(e) e
    )
    
    # Evaluate model if successful
    if (!inherits(model, "try-error")) {
    ...
    # Save results for the current formula and fold
    qs::qsave(aggregated_results, output_path)
  }
}
...
```{r}
 ...
 #     if(file.exists(here("int/models/diagnostics/cv_results/"))) need to figure out how to skip formula that is already done
    ...
    fold_results <- do.call(rbind, lapply(seq_along(cv_models), function(idx) {
      m <- cv_models[[idx]]
      if (!inherits(m, "try-error")) {
        ...
        mse <- mean((test$trim_prop_beta - pred)^2, na.rm = TRUE)
        mae <- mean(abs(test$trim_prop_beta - pred), na.rm = TRUE)
        data.frame(model_formula = choose_models[idx], mse = mse, mae = mae)
      } else {
        data.frame(model_formula = choose_models[idx], mse = NA, mae = NA)
      }
    }))
    
    
      # Update aggregated results
    for (formula in unique(fold_results$model_formula)) {
      
      # formula = "trim_prop_beta ~ filleted"
      fold_data <- fold_results[fold_results$model_formula == formula, ]
      
      # Check if this model formula exists in aggregated results
      existing_row <- which(aggregated_results$model_formula == formula)
      
      if (length(existing_row) == 0) {
        # Add new row
        aggregated_results <- rbind(aggregated_results, data.frame(
          model_formula = formula,
          mean_mse = fold_data$mse,
          mean_mae = fold_data$mae,
          n_folds = 1,
          stringsAsFactors = FALSE
        ))
      } else {
        # Update existing row
        current <- aggregated_results[existing_row, ]
        n <- current$n_folds
        aggregated_results$mean_mse[existing_row] <- 
          (current$mean_mse * n + fold_data$mse) / (n + 1)
        aggregated_results$mean_mae[existing_row] <- 
          (current$mean_mae * n + fold_data$mae) / (n + 1)
        aggregated_results$n_folds[existing_row] <- n + 1
      }
      qs::qsave(aggregated_results, here(glue("int/models/diagnostics/cv_results/{formula}_fold_{k}_{i}_results.qs")))
      
    }
  }
   
   
final_cv_results <- lapply(list.files(here("int/models/diagnostics/cv_results/"), full.names = TRUE), qread) %>%
  bind_rows() %>%
  group_by(model_formula) %>%
  summarise(mean_mse = mean(mean_mse, na.rm = TRUE), 

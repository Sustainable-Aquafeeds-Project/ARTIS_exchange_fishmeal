...
2. Model Fitting GAMs with beta distribution
...
ctrl <- list(nthreads=8) # try with 8 cores?

tic()
gam_model_beta <- gam(trim_prop_beta ~ s(price_gf) + filleted  +
                 source_country_iso3c + family + year, 
                 data = artis_trimmings_beta_2, family = betar(), control = ctrl) # production level? Relative contribution of that species production for that country/year

qs::qsave(gam_model_beta, here("int/gam_model1_beta.qs"))
toc() # 12 hours with 1 core. ~6 hours with 6 cores

tic()
gam_model_beta2 <- gam(trim_prop_beta ~ s(price_gf) + year + filleted +
                  source_country_iso3c, 
                  data = artis_trimmings_beta_2, family = betar(), na.action = na.exclude, control = ctrl)


qs::qsave(gam_model_beta2, here("int/gam_model2_beta.qs"))
toc() # 20 mins with 4 cores! It takes 1 hour with 1 core. 
...
toc() # 5 hours on one core, 1.5 hours on 8
...
gam_model_beta <- qread(here("int/gam_model1_beta.qs"))
...
#                 df <dbl>   AIC <dbl>
# gam_model_beta	389.85897	-1530793		
# gam_model_beta2	194.99425	-1769291		
# gam_model_beta3	217.77126	-1529392		
# gam_model_beta4	12.99493	-1765623	

logLik(gam_model_beta) # 'log Lik.' 765786.4 (df=389.859)

logLik(gam_model_beta2) # 'log Lik.' 884840.6 (df=194.9943)

logLik(gam_model_beta3) # 'log Lik.' 764913.8 (df=217.7713)
 
logLik(gam_model_beta4) # 'log Lik.' 882824.5 (df=12.99493)
...
all_data_gf <- predict_data_all_gf_fin %>%
  ...
  left_join(artis_taxa) %>%
    dplyr::select(year, source_country_iso3c, exporter_iso3c, consumer_iso3c, dom_source, sciname, common_name, habitat, method, trim_prop_gf_fin, trim_liveweight, trim_product, wholefish_liveweight, wholefish_product, phylum, method_trim_gf = trim_prop_gf_flag) %>%
...
artis_all_2020 <- all_data_gf %>%
  filter(year == 2020,
         str_detect(method_trim_gf, "GAM")) %>%
  distinct(source_country_iso3c, sciname, common_name, phylum, trim_prop_gf_fin)

unique(all_data_gf$method_trim_gf)
```
...
```{r}
test <- read.csv(file.path("/mnt/rdsi/raw_data/fao/processed_products/CL_FI_COMMODITY_ISSCFC.csv"))

# Fishmeal obtained from fish waste/offals
# Code = 081.42.1.5.9.99
# parent code = 081.42.1.5.9
# ISSCAAP = 39
# identifier = 1602


countries <- read.csv("/mnt/rdsi/raw_data/fao/processed_products/CL_FI_COUNTRY_GROUPS.csv") %>%
  clean_names()

production <- read.csv(file.path("/mnt/rdsi/raw_data/fao/processed_products/TRADE_PP_QUANTITY.csv")) %>%
  clean_names() %>%
  filter(commodity_fao_code == "081.42.1.5.9.99") %>%
  left_join(countries, by = c("country_un_code" = "un_code"))

# only Chile and Peru are reporting.. so not much to go by from here. Could get trends from these countries and then apply it worldwide i suppose. 
# big increase in Peru from 2017-2022
# no increase in Chile from 1985-2017?

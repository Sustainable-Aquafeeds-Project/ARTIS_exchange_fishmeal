...
cao_int <- read.csv(here("int/material_origins_cao_chn.csv")) %>%
    mutate(trimmings_tonnes = NA, wholefish_tonnes = NA) %>%
  left_join(artis_genus_df, by = "common_name") %>%
  mutate(sciname.y = ifelse(is.na(sciname.y), sciname.x, sciname.y)) %>%
  mutate(sciname.x = ifelse(sciname.x != sciname.y , sciname.y, sciname.x)) %>%
  dplyr::select(-sciname.y) %>%
  rename(sciname = sciname.x) %>%
  dplyr::select(1:14)

  
all_int_dfs <- rbind(biomar_int, cargill_int, skretting_int, cao_int) %>%
  mutate(sciname = tolower(sciname),
...
write.csv(all_int_dfs, here("int/material_origins_biomar_cargill_skretting_cao.csv"), row.names = FALSE)
...
int_dfs_summary <- all_int_dfs %>%
  ...
  dplyr::select(sciname, common_name, iso3c, capture, trash_fish, trim_prop = trim_prop_w_mean) %>%

...
cross validation 
 - Choose best models? I want to cut this down as much as possible, because some of these models take really long to run (~7-8 hours), and running them 5 times might take quite a long time. 
  - NOTE: Dec 19, 2024 - for some reason I can't get any of the models with "family" variable in it to run. Not sure why this is happening, because it worked above. Regardless, those are all the worst performing models according to AIC and BIC, so it probably doesn't matter. 

```{r}
ctrl <- list(nthreads=6) # 20 cores?
...
done_forms <- lapply(list.files(here("int/models/diagnostics/cv_results/"), full.names = TRUE), qs::qread) %>%
  bind_rows() %>%
  distinct(model_formula) %>%
  pull()

choose_models <- setdiff(choose_models, done_forms)

choose_models <- choose_models[[16]]
...
for (i in 1:k) {
  ...
  for (formula in choose_models) {
    ...
    model <- gam(as.formula(formula), data = train, family = betar(), control = ctrl, na.action = na.exclude)
...
results <- lapply(list.files(here("int/models/diagnostics/cv_results/"), full.names = TRUE), qs::qread) %>%
  bind_rows() %>%
  group_by(model_formula) %>%
  summarise(mse = mean(mean_mse, na.rm = TRUE),
            mae = mean(mean_mae, na.rm = TRUE)) %>% 
  ungroup()

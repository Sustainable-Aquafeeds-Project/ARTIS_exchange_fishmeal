...
This script also gapfills data from the last script. We split it into two parts, because it was getting a bit unweildy and we use more sophisticated methods for this (e.g., the ex vessel price data)
...
test <- fillet_artis %>% 
  ...
  filter(source_country_iso3c == exporter_iso3c) # ok so there is not domestic production in this? Or at least there is not any non-traded production
...
tonnes_filled_price/fm_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # 0.02666722 ~ 2.6%..
 ...
 price_all_gf %>% 
  ...
  filter(year == 2020) %>%
  pull(product_weight_t) %>%
  sum()/fm_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # 96.6% now filled in
...
artis_trimmings_all_info_new %>% 
  filter(year == 2020) %>%
  pull(product_weight_t) %>%
  sum()/fm_artis %>% filter(year == 2020) %>% pull(product_weight_t) %>% sum() # ok now we have 98.9% of fishmeal for 2020 covered
...
artis_trimmings_all_gf %>% 
  filter(
         !is.na(trim_prop_gf)) %>%
  pull(product_weight_t) %>%
  sum()/fm_artis %>% pull(product_weight_t) %>% sum() # 0.9901832

...
fm_origins_df_test <- read.csv(here("outputs/all_fmfo_material_origins_reports.csv")) %>%
  dplyr::select(-common_name) %>%
...
summary(artis_trimmings_all_traded)

# 3738281/4075045 # missing 91% of rows. 
# # how much fishmeal? 

# sum(artis_trimmings_all_traded$product_weight_t)
# test <- artis_trimmings_all_traded %>%
#   filter(!is.na(trim_prop)) %>%
#   pull(product_weight_t) %>%
#   sum()
# 
# test/sum(artis_trimmings_all_traded$product_weight_t) # got about 24% of fishmeal this way! 
# 
# # what about just for 2020? 
# 
# test <- artis_trimmings_all_traded %>%
#   filter(!is.na(trim_prop),
#          year == 2020) %>%
...
#   sum()
# 
# artis_trimmings_all_traded %>%
#   filter(year == 2020) %>%
#   pull(product_weight_t) %>%
#   sum()

# test/6158133 # got about 28% of fishmeal 2020 this way! That's pretty good!  
...
artis_trimmings_all_info <- artis_trimmings_info_sources %>%
  ...
  dplyr::select(-trim_prop) %>%
  mutate(method_trim_gf = "none") # ok this is all the non-gapfilled data from our literature review
...
artis_trimmings_info_fixed_1 <- artis_trimmings_info_to_fix_1 %>%
  ...
  mutate(method_trim_gf = "Average trimmings proportion of source country and species from literature and reports")
...
ggplot(test_long, aes(x = source, y = tonnage, fill = isscaap)) +
  ...
  theme(legend.position = "right") # cool, mostly NEI species, which makes sense.
...
```{r}
test <- artis_trimmings_info_to_fix_7 %>%
